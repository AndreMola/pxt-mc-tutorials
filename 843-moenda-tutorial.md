...
## Passo 4
Refaça o mesmo processo anterior para criar e definir mais duas variáveis: **etanol** e **açúcar**, 
...
## Passo 6
Agora atualize os campos das multiplicações. Defina açúcar para ser o produto 
entre a variável ``||variables:total||`` e o valor **120** e depois defina etanol para ser o produto 
...
pegue **três** blocos ``||serial:serial gravar linha ""||`` para colocar dentro 
do comando ``||input:no logotipo pressionado||``.
...
## Passo 10
Clique no botão **+** do texto ``||text:unir Olá Mundo - +||`` para adicionar mais um campo no bloco.
...
``||actuators:Motor CC, para na porta P8||`` dentro da função ``||functions: parar||``.
Modifique a porta de **P8** para **P16**.
...
Modifique a porta de **P8** para **P16** e a velocidade para **1023**.
...
Na categoria ``||variables:Variáveis||`` pegue o bloco ``||variables:alterar total por 1||``
e jogue-o para o final da função.
...
```
## Passo 20
Mude a porta para **P12** e substitua o valor **0** da posição por uma
...
acesse a categoria ``||sensor:Sensores|`` e atualize o primeiro campo da divisão com
o bloco ``||sensor:Valor do potenciômetro na porta P2|``. Altere a porta de **P2** para **P1**.
O segundo campo passará a ser o número **2**.
...
```
## Passo 23
Volte ao menu ``||logic:Lógica||``, pegue o booleano ``||logic: e ||``
...
```package
smartmakers=github:AndreMola/make_code_core
```